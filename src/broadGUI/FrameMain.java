...
public class FrameMain extends JFrame {
	...
	private static JPanel panelAdminBBMG() {
        ...
        JButton tvProUpdBtn= new JButton("프로그램 등록");
		...
		addComponent(BBMG, prBox, 0, 0, gbc);
		addComponent(BBMG, tvBox, 0, 1, gbc);
		addComponent(BBMG, dayBox, 0, 2, gbc);
		addComponent(BBMG, tvProUpdBtn, 0, 3, gbc);
		addComponent(BBMG, backButton, 0, 4, gbc);
	...
	private static void GRDEL(JTextField grName) {
    	...
    	int result = JOptionPane.showConfirmDialog(mainPanel, String.format("%s 삭제 하시겠습니까?", grNameText), "확인", JOptionPane.YES_NO_OPTION);
		if (result == JOptionPane.NO_OPTION) {
			return;
		}
		// grNameText 인 프로그램 삭제 -> 
		// 결과 받아올수 있으면 int result = JOptionPane.showConfirmDialog(mainPanel, String.format("%s 삭제되었습니다.", grNameText), "확인", JOptionPane.YES_NO_OPTION);
	...
	private static void GRADDUP(JTextField grName, JTextField grName2) {
		...
		if (result == JOptionPane.NO_OPTION) {
			int result2 = JOptionPane.showConfirmDialog(mainPanel, "기존 장르명 수정 하시겠습니까?", "확인", JOptionPane.YES_NO_OPTION);
	...
	private static void TVDEL(JTextField tVName) {
    	...
    	int result = JOptionPane.showConfirmDialog(mainPanel, String.format("%s 삭제 하시겠습니까?", tvNameText), "확인", JOptionPane.YES_NO_OPTION);
		if (result == JOptionPane.NO_OPTION) {
			return;
		}
		// tvNameText 인 프로그램 삭제 -> 
		// 결과 받아올수 있으면 int result = JOptionPane.showConfirmDialog(mainPanel, String.format("%s 삭제되었습니다.", tvNameText), "확인", JOptionPane.YES_NO_OPTION);
	...
	private static void TVADDUP(JTextField tVName, JTextField tVName2) {
		...
		if (result == JOptionPane.NO_OPTION) {
			int result2 = JOptionPane.showConfirmDialog(mainPanel, "기존 방송사 수정 하시겠습니까?", "확인", JOptionPane.YES_NO_OPTION);
				if (result2 == JOptionPane.NO_OPTION) return;
	...
	private static Component panelPRGR() {
	...
	private static Component panelPRADD() {
	...
	private static void PRDEL(JTextField pRName) {
    	...
    	int result = JOptionPane.showConfirmDialog(mainPanel, String.format("%s 삭제 하시겠습니까?", pRNameText), "확인", JOptionPane.YES_NO_OPTION);
		if (result == JOptionPane.NO_OPTION) {
			return;
		}
		// pRNameText 인 프로그램 삭제 -> 
		// 결과 받아올수 있으면 int result = JOptionPane.showConfirmDialog(mainPanel, String.format("%s 삭제되었습니다.", pRNameText), "확인", JOptionPane.YES_NO_OPTION);
	
	}

	private static void PRADDUP(JTextField pRName, JTextField pRName2, JTextField pRAge, JTextField pRAge2) {
		...
		if (result == JOptionPane.NO_OPTION) {
			int result2 = JOptionPane.showConfirmDialog(mainPanel, "기존 프로그램을 수정 하시겠습니까?", "확인", JOptionPane.YES_NO_OPTION);
				if (result2 == JOptionPane.NO_OPTION) return;
					if(pRName2Text.isBlank()) {
						if(pRAge2Text.isBlank()){
				    		JOptionPane.showMessageDialog(mainPanel, "이름과 연령대 둘 다 공란일 경우 수정이 불가합니다.");	
				    		return;
						}
    ...
    private static JPanel panelBookmarkDel() {
...
*/
	private static JPanel panelBookmarkAdd() {
    ...
    private static void registerIdUpdate(JTextField idText2, JTextField pwText2, JTextField pw2Text2, JTextField nameText2) {
        ...
        try {
        	...
        	// 여기서 string들 보내고
            // if (/*바꾸는 id가 이미 존재하면 -> unique 속성 때문에 추가 안됨*/){ JOptionPane.showMessageDialog(mainPanel, "이미 존재하는 아이디.");	return;}
